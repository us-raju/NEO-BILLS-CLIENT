...
const Register = () => {
  const { setUser, createUser, signInWithGoogle, updateProfileData } =
    useAuth();
  ...
  const handleFormSubmit = (e) => {
    ...
    // password valitaiton
    const strongRegex =
      /^(?=\S*$)(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^A-Za-z0-9]).{6,}$/;
    if (password.length < 6) {
      return setError("Password must be 6 characters");
    }
    if (!strongRegex.test(password)) {
      setError(
        "Password must be at least 6 characters and include Upercase and Lowercase letters, numbers, and special characters."
      );
      return setError;
    }
    setError("");

    // signUp with email and password functionality here
